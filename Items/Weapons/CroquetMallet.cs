...
{
	public class CroquetMallet : ResilienceDamage
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Croquet Mallet");
			Tooltip.SetDefault("Thwack a mini red ball at your enemies!");
		}
		public override void SafeSetDefaults() {
			...
			item.value = 200; // how much the item sells for (measured in copper)
			item.rare = ItemRarityID.Blue; // the color that the item's name will be in-game
			item.UseSound = SoundID.Item14; // The sound that this item plays when used.
			item.autoReuse = true; // if you can hold click to automatically use it again
			item.shoot = ModContent.ProjectileType<MiniRedBallProj>(); //idk why but all the guns in the vanilla source have this
