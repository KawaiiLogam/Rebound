...
namespace BounceBack.NPCs
{
	...
	[AutoloadHead]
	public class Teacher : ModNPC
	{
		public override string Texture => "BounceBack/NPCs/Teacher";
		...
		public override void SetStaticDefaults() {
			...
			DisplayName.SetDefault("Teacher");
		...
		public override bool CanTownNPCSpawn(int numTownNPCs, int money)
		{
			for (int k = 0; k < 255; k++) {
				...
				foreach (Item item in player.inventory) {
					if (item.type == ModContent.ItemType<Pencil>()) {
						return true;
					}
				}
			}
		...
		public override string GetChat() {
			...
			if (PartyGirl >= 0 && Main.rand.NextBool(5)) {
				return "Have you seen " + Main.npc[PartyGirl].GivenName + "? She's supposed to be my teacher's assistant.";
			}
			if (Guide >= 0 && Main.rand.NextBool(5)) {
				return "I taught " + Main.npc[Guide].GivenName + " everything he knows. He can't retain information really well but for some reason he is a wiz at remembering crafting recipes.";
			}
			switch (Main.rand.Next(6)) {
				case 0:
					return "I hope you're coming to class tomorrow.";
				case 1:
					return "My main profession is school teacher. However I also like to practice Vivva Magic in my spare time.";
				case 2:
					return "These youngsters and their 'gacha' games. It seems harmless now but wait until they find out about casinos.";
				case 3:
					return "OW! I keep banging my big toe against the furniture in here.";
				case 4:
					return "You can be such a big toe.";
				default:
					return "The internet really is changing things. For instance I don't think anyone here needs sex ed";
			}
		}
		...
		*/
		...
		public override void SetupShop(Chest shop, ref int nextSlot) {
			shop.item[nextSlot].SetDefaults(ModContent.ItemType<ClassroomChair>());
			nextSlot++;
			shop.item[nextSlot].SetDefaults(ModContent.ItemType<ClassroomDesk>());
			nextSlot++;
			shop.item[nextSlot].SetDefaults(ModContent.ItemType<Locker>());
			nextSlot++;
			shop.item[nextSlot].SetDefaults(ModContent.ItemType<Chalkboard>());
			nextSlot++;
			shop.item[nextSlot].SetDefaults(ModContent.ItemType<Pencil>());
			/*nextSlot++;
